#include "sv_RequestHandler.h"

#include "../Utils/utils_CompileTime.h"
#include <spdlog/spdlog.h>

namespace {

    template <typename T, http::verb method>
    concept has_handle_impl = requires(T t, request::Request& req, request::Response& res) {
    ...
    using request_handler_operation = void (*)(request::Request&, request::Response &);

    using array_type = std::array<std::array<request_handler_operation, enum_size<http::verb>()>, enum_size<request::Target>()>;

    consteval array_type generate_array() {
        array_type array{nullptr};

        for_<enum_size<http::verb>()>([&] (const auto i) {
