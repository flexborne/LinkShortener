...
struct ServerConfig {
    ...
    static inline const auto ip_address = tcp::v4();
};
...
private:
    ...
    void do_write() {
        ...
        beast::http::async_write(stream_, response_,
                                 [self](beast::error_code ec,
                                        std::size_t bytes_transferred) {
                                     if (ec) {
...
void do_accept(tcp::acceptor& acceptor,
    ...
    acceptor.async_accept([&](beast::error_code ec, tcp::socket&& socket) {
        if (!ec) {
            ...
            std::make_shared<Session>(std::move(socket), ssl_context)
                    ->start();
        } else {
            spdlog::error(ec.what());
        }
...
int main() try {
    ...
    asio::io_context ioc;
    tcp::acceptor acceptor{ioc, {ServerConfig::ip_address, ServerConfig::port_num}};
